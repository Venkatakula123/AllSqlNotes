...
select date_format(created_at,'%Y-%m') as ym, sum(value) from sf_transactions group by date_format(created_at,'%Y-%m');

select date_format(created_at,'%Y-%m') as ym, sum(value) as total from sf_transactions group by ym;

with cts AS (select date_format(created_at,'%Y-%m') as ym, sum(value) as total from sf_transactions group by ym) 
select ym, ROUND(total - LAG(total) over(order by ym) * 100.0 / LAG(total) over(order by ym),2) as pct  from cts;
...
create table d_countries like countries;

select * from d_countries;

show tables like 'dup_%';
